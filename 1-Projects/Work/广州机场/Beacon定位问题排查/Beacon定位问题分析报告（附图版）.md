...
## 现象三：10002 的Beacon对定位存在一定干扰

### 数据流说明

定位链路为：定位工牌扫描周围Beacon → 工牌将扫描到的Beacon信息通过BLE广播发出 → AP接收工牌的广播数据 → 透传给服务器 → 服务器根据Beacon信息计算位置。

### 观察

现场工牌扫描到了 major=10002 的未知Beacon信号：

![现场收到未知Beacons信号major=10002](Attachments/现场收到未知Beacons信号major=10002.png)

定位工牌在扫描周围Beacon时，除了扫到我们部署的 major=2504 系列定位Beacon，偶尔还会扫到 major=10002 的未知Beacon信号。

在同时间段日志中，设备 `c300005b2294` 正常解析出的beacon信息都是 `2504:xxxx` 系列（如 2504:1128、2504:1123、2504:1133、2504:1098 等），这些是我们部署的定位Beacon。但工牌在扫描环节偶尔会把 10002 的Beacon数据也一并采集，混在广播包中通过AP透传到服务端。

### 影响分析

- 10002 的Beacon不在我们的定位Beacon数据库中，服务端在匹配时会忽略，不会直接参与定位计算
- 但工牌的BLE扫描能力有限，每个广播周期能携带的Beacon数据条数有上限。当工牌扫到10002的Beacon时，可能会挤占原本应该上报的2504系列有效Beacon的位置
- 从日志中可以看到，工牌每次上报的Beacon数量通常为3~4个，如果其中一个位置被10002占据，有效定位Beacon就少了一个，可能影响定位精度
- 如果10002的Beacon信号较强，工牌可能优先采集它而丢弃信号较弱的有效Beacon

### 结论

10002 的Beacon确实对定位有一定影响，主要体现在占用工牌有限的BLE扫描和广播资源，可能挤占有效的2504系列Beacon数据。建议排查10002 Beacon的来源（可能是现场其他系统的设备），如果是非必要设备考虑关闭；或者在工牌固件侧增加Beacon过滤规则，只扫描 major=2504 的Beacon。
...
| 工牌扫描 | ⚠️ 有干扰 | 偶尔扫到10002未知Beacon，挤占有效数据 |
...
2. **过滤10002 Beacon**：在工牌固件侧增加Beacon过滤规则，只扫描 major=2504 的Beacon，避免无效Beacon挤占有限的广播数据位；同时排查10002 Beacon的来源
3. **考虑多AP冗余**：从同时间段日志可以看到，同一设备被多个AP（`9848747a17b0`、`9848747a2a90`、`98487479eb50`）同时接收到，多AP冗余可以在一定程度上弥补单AP漏报的问题
4. **监控告警**：建议增加AP上报数据连续性的监控，当某AP连续多个周期无数据时触发告警
...
| [现场收到未知Beacons信号major=10002.png](Attachments/现场收到未知Beacons信号major=10002.png) | 现象三：10002 Beacon干扰截图 |
