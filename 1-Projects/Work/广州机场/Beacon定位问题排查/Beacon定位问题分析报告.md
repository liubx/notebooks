...
从截图（现场收到未知Beacons信号major=10002.png）可以看到，定位工牌在扫描周围Beacon时，除了扫到我们部署的 major=2504 系列定位Beacon，偶尔还会扫到 major=10002 的未知Beacon信号。
...
从同一AP（`9848747a17b0`）连续接收数据日志中，按时间排列分析数据间隔：
...
```
定位Beacon广播 → 定位工牌扫描周围Beacon → 工牌BLE广播（携带Beacon数据） → AP接收工牌广播（10s汇聚） → AP透传上报（UDP） → 服务端接收解析（~2ms） → Beacon匹配（~1ms） → 位置计算与发送（~2ms）
```
...
| AP BLE扫描 | ✅ 正常 | 每周期扫描到 15~27 个标签 |
| AP数据发送 | ✅ 正常 | 严格10秒间隔，原始抓包确认 |
| AP→服务端传输 | ⚠️ 有丢包 | 约25%的周期数据未到达服务端 |
| 服务端处理 | ✅ 正常 | 毫秒级处理延迟 |
| 10002 Beacon干扰 | ⚠️ 有影响 | 工牌扫描到未知Beacon，挤占有效数据 |
