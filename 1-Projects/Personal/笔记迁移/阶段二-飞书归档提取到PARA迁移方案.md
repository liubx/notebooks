...
- `项目日志`：所有项目统一包含
- **严格按模板结构**，不要添加模板中没有的部分（如"相关笔记"）
- 本地有的文件（md 和 file 附件）都用 `[[wikilink]]`
- ☁️ 仅云端 = 飞书有、本地不保留（3D模型/设计文件/视频等大文件）
- 📌 仅本地 = 本地有、飞书没有
- 无标记 = 双端都有
- 文件少于 30 个时平铺不建子文件夹，超过 30 个再考虑

**文件清单章节标题规则**：
- 本地有的目录（如 `项目文档/`）→ 总览中必须有 `### 项目文档` 章节标题
- 本地没有的文件 → 按内容分组显示（如 `### 技术文档`、`### 项目管理`、`### 演示与媒体`）
- `Attachments/` 不列入文件清单

**文件记录规则**：
- 本地有 md 的 → 用 `[[wikilink]]` + 飞书知识库链接
- 本地有 file 附件的（.docx/.xlsx/.pptx/.pdf/.jpeg 等）→ 用 `[[wikilink]]` + 飞书知识库链接 + 📌 本地有
- 本地有 sheet 导出的 xlsx → 用 `[[wikilink]]` + 飞书知识库链接 + 📌 本地有 xlsx
- 只在飞书知识库的（3D模型/设计文件/视频/大文件等）→ 文件名 + ☁️ 仅云端 + 飞书知识库链接
- 只在本地的 → 用 `[[wikilink]]` + 📌 仅本地
- 子目录整体记录 → 目录名 + 文件数量说明 + 飞书知识库链接
- **不记录**只在飞书云空间原始目录中、不在知识库也不在本地的文件
- **所有飞书文件都要迁移到知识库**，不能只迁移部分文件

**飞书链接规则**：
- 飞书链接指向知识库节点（`/wiki/` 格式），不指向云空间原始目录
- 子目录链接指向云空间新目录（`/drive/folder/` 格式）
...
- [ ] **frontmatter 完整**：title、type、category、status、tags、created、modified 都有
- [ ] **飞书云空间链接正确**：folder token 是该项目自己的（不是其他项目的），通过 `drive files list` 验证
- [ ] **飞书知识库链接正确**：node_token 指向知识库中该项目的根节点
- [ ] **本地归档目录路径正确**：指向 `4-Archives/Notes/Feishu/...` 中的实际路径
- [ ] **文件清单按知识库分类组织**：与知识库子节点结构一致
- [ ] **所有飞书链接指向知识库**：使用 `/wiki/{node_token}` 格式，不是 `/docx/` 或 `/file/` 格式
...
- [ ] **根节点位置正确**：在"归档项目"节点下（已结束项目）或知识库根目录（进行中项目），用 `wiki nodes get` 验证 `parent_node_token` 是否为归档项目节点（`PhpuwDqUvidfKRkUXMKcLMJ0nHc`）
- [ ] **子分类节点已创建**：按项目文件夹结构创建子节点
- [ ] **所有文件已 drive copy + move_docs_to_wiki**：包括照片、视频、源码包等所有 file 类型
- [ ] **无遗漏文件**：对比飞书云空间原始目录和知识库节点数量
- [ ] **所有目录页（根节点+每个子分类）都已写入 mention-doc 格式**：迁移完成后必须逐个检查每个目录节点，不能只检查根节点
...
- [ ] **迁移记录.md 已创建**：包含知识库结构树、统计数据、待清理项
- [ ] **旧文件已清理**：如果有旧的单文件归档（如 `博世小程序项目.md`），已合并到新目录并删除旧文件

### 五、提交推送检查

- [ ] **git add -A**：所有变更已暂存
- [ ] **commit message 清晰**：说明迁移了哪个项目
- [ ] **git push 成功**：大文件（mp4/zip 等）可能需要较长时间，用 `controlBashProcess` 后台推送
- [ ] **推送后验证**：`git status` 显示 clean

### 常见错误提醒

1. **飞书链接用了云空间而非知识库**：所有链接必须是 `/wiki/{node_token}` 格式
2. **folder token 复制错误**：每个项目有自己的 folder token，不要复制其他项目的
3. **本地文件不全**：必须用 rsync 全量同步，不能只复制 md 和部分附件
...
13. **目录页用普通链接代替 mention-doc**：如 `[文件名](https://reliablesense.feishu.cn/wiki/xxx)` 不是 mention-doc 格式。必须用 `<mention-doc token="obj_token" type="类型">文件名</mention-doc>` 格式，mention-doc 写入是可行的（用 `docs +update --mode overwrite`），关键是使用正确的 **obj_token**（不是 node_token）
14. **根节点目录页有描述行或旧分类链接**：根节点只能有 `# 项目名` + mention-doc 子分类列表，不能有项目描述（如"xxx项目文档归档"）、不能有 `## 文档分类` 等二级标题、不能有"旧分类（已清空）"等历史信息
15. **子节点目录页有描述行**：子节点只能有 `# 分类名` + mention-doc 文件列表，不能有分类描述（如"md 技术文档（去重后保留最新版）"）
16. **迁移后未回头检查所有目录页**：每次迁移完成后，必须用 `docs +fetch` 逐个检查根节点和每个子分类节点的目录页，确认都是 mention-doc 格式、无二级标题、无描述行。不能只写入不验证
17. **临时文件未清理**：迁移过程中生成的临时脚本（.sh）、日志（.log）、结果文件（.tsv）、临时 markdown（.md）必须在提交前删除
