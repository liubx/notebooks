...
## [LRN-20260416-001] 同步任务时必须主动补全缺失字段

**时间**: 2026-04-16
**领域**: feishu, task-sync

### 学到了什么
同步任务时，如果飞书端缺少状态/优先级/负责人，不应该留空等用户指定，而应该：
1. 根据任务内容和类型主动估算并补全
2. 根据团队分工模式自动分配负责人（后端→姚文羚，前端→闫云江，测试→陆东，运维→钟申炜，告警规则→刘秉欣）
3. 根据任务紧急程度估算优先级
4. 根据任务进展估算状态（新任务→waiting，已开始→doing，测试中→testing）
5. 补完后立即同步到飞书，不需要逐个问用户确认

### 之前的错误做法
- 查到飞书端没设就直接留空，报告"飞书端确实没设"
- 每个字段都问用户确认，浪费大量来回
- 把"确认过飞书端没设"理解为"可以不填"

### 正确做法
同步时一次性完成：拉取→对比→补全缺失字段→更新飞书→更新本地。整个过程自动执行，只在有歧义时才问用户。
