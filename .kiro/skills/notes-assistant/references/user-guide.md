...
2. 文件在 `1-Projects/` → 项目 | `2-Areas/Work/` → 工作 | `2-Areas/Personal/` → 个人
3. 无标记 → 默认个人
...
需要先安装并配置 lark-cli（`npm install -g @larksuite/cli && lark-cli config init`）。
...
# 验证
lark-cli task +get-my-tasks --page-limit 2
```
