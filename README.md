...
基于 Kiro Steering + PARA + Zettelkasten 的 Obsidian 知识管理系统。无需安装任何依赖，AI 直接读取规则文件并操作 Vault。

## 工作原理

```
用户对话 → AI 读取 Steering 规则 → AI 直接读写 Vault 文件
                                  → AI 调用 lark-mcp 同步飞书
```

所有业务逻辑以 Markdown 规则文件的形式存储在 `.kiro/steering/` 下，AI 按规则执行操作，不依赖任何脚本或代码。
...
├── .kiro/
│   ├── steering/                          # AI 规则文件
│   │   ├── notes-assistant-workflow.md    # 主工作流（结构、笔记、任务、回顾、归档）
│   │   ├── notes-assistant-sync.md        # 飞书同步规则
│   │   ├── notes-assistant-templates.md   # 笔记模板定义
│   │   └── notes-assistant-improvements.md # 用户偏好记录
│   ├── settings/
...
在 Kiro 聊天中输入 `#notes-assistant-workflow` 引入主工作流规则，AI 即可开始工作。

按需引入其他规则：
- `#notes-assistant-templates` — 需要创建笔记时
- `#notes-assistant-sync` — 需要飞书同步时
- `#notes-assistant-improvements` — 需要 AI 参考你的偏好时
...
所有模板定义在 `notes-assistant-templates.md` 中，AI 创建笔记时直接使用：
...
- 修改 `.kiro/steering/notes-assistant-workflow.md` 可调整文件夹结构、分类规则等
- 修改 `.kiro/steering/notes-assistant-sync.md` 可调整同步判断逻辑和关键词
- 修改 `.kiro/steering/notes-assistant-templates.md` 可调整模板内容
- `.kiro/steering/notes-assistant-improvements.md` 会随使用逐步积累你的偏好
